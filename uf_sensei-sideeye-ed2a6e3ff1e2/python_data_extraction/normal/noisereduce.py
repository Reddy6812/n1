...
def subRSNoise(data, rate, noise_dur=0.1, trim_red = 0.1):
    ...
    for i_ch in range(data.shape[1]):
        ...
        new_data[:, i_ch] = new_sig
        yt, trim_idx_all[i_ch, :] = librosa.effects.trim(new_data[:, i_ch], top_db=20, frame_length=4096,
                                                         hop_length=2048)

    trim_start = np.max([0, int(np.mean(trim_idx_all[:, 0]) - rate * trim_red)])
    trim_end = np.min([new_data.shape[0], int(np.mean(trim_idx_all[:, 1]) + rate * trim_red)])
