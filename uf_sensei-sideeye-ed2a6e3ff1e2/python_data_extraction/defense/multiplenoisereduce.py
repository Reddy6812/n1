...
def subRSNoise(data, rate, noise_dur=0.2, trim_red = 0.1, iter=8):
    
    
    trim_idx_all = np.zeros((data.shape[1], 2))
    temp_data = np.zeros(data.shape, dtype=np.float)
    temp_data = copy.deepcopy(data).astype('float')
    # pprint(temp_data.shape, temp_data.dtype)
    for iter_index in range(iter):
        for i_ch in range(data.shape[1]):
            sig = temp_data[:, i_ch]
            new_sig = reduce_noise(y=sig, sr=rate, y_noise=sig[:int(rate * noise_dur)], stationary=True)
            temp_data[:, i_ch] = new_sig / abs(np.max(new_sig)) if abs(np.max(new_sig)) != 0 else new_sig 

    for channel_index in range(data.shape[1]): 
        yt, trim_idx_all[channel_index, :] = librosa.effects.trim(temp_data[:, channel_index], top_db=20, frame_length=4096,
                                                                  hop_length=2048)

    trim_start = np.max([0, int(np.mean(trim_idx_all[:, 0]) - rate * trim_red)])
    trim_end = np.min([temp_data.shape[0], int(np.mean(trim_idx_all[:, 1]) + rate * trim_red)])

    new_data = temp_data[trim_start:trim_end, :]
