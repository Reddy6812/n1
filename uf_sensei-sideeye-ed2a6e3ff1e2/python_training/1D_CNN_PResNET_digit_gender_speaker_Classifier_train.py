...
class ResNetResidualBlock(ResidualBlock):
    def __init__(self, in_channels, out_channels, expansion=1, downsampling=1, conv=conv3x3, *args, **kwargs):
        ...
        {
            'dropout' : nn.Dropout(0.30),
            'conv' : nn.Conv1d(self.in_channels, self.expanded_channels,
...
def conv_bn(in_channels, out_channels, conv, *args, **kwargs):
    return nn.Sequential(OrderedDict({ 'dropout' : nn.Dropout(0.30),
...
class ResNetLayer(nn.Module):
    def __init__(self, in_channels, out_channels, block=ResNetBasicBlock, n=1, *args, **kwargs):
        ...
        downsampling = 8 if in_channels != out_channels else 5
...
def train_a_1D_CNN_signal_classifier(BASE_PATH, DATA_PATH, logging_instant, device_instant, num_classes, label_index):    
    num_workers = 16
    EPOCHS = 5000
    BATCH_SIZE = 64
    VALID_BATCH_SIZE = 64
    ...
    train_set = SignalDataset(DATA_PATH, VALID_BATCH_SIZE, data_type='train', label_index=label_index)
    test_set = SignalDataset(DATA_PATH, VALID_BATCH_SIZE, data_type='test', label_index=label_index)
    valid_set = SignalDataset(DATA_PATH, VALID_BATCH_SIZE, data_type='valid', label_index=label_index)
...
def test_a_1D_CNN_signal_classifier(BASE_PATH, DATA_PATH, logging_instant, device_instant, num_classes, label_index):    
    num_workers = 16
    EPOCHS = 5000
    BATCH_SIZE = 64
    VALID_BATCH_SIZE = 64
...
def spun_train_one_1D_CNN_signal_classifier(device_instant, main_logger, type_label, num_classes, label_index):
    BASE_PATH = f'/blue/srampazzi/pnaghavi/test30/1D_CNN_signal_classifier_{type_label}'
    ...
    try:
        final_model, device = train_a_1D_CNN_signal_classifier(BASE_PATH, DATA_PATH, training_instant_logger, device_instant, num_classes, label_index)
    except Exception as ex:
        main_logger.exception(ex)


def spun_test_one_1D_CNN_signal_classifier(device_instant, main_logger, type_label, num_classes, label_index):
    BASE_PATH = f'/blue/srampazzi/pnaghavi/test30/1D_CNN_signal_classifier_{type_label}'
    if not os.path.exists(BASE_PATH):
        os.makedirs(BASE_PATH)
        main_logger.info(f'Created BASE_PATH {BASE_PATH} for label instant {type_label}')
    ...
    main_logger.info('Created logger instance with path ' + BASE_PATH + '/' + type_label + '_log.log' + 'for label instant ' + type_label)
    try:
        final_model, device = test_a_1D_CNN_signal_classifier(BASE_PATH, DATA_PATH, training_instant_logger, device_instant, num_classes, label_index)
    except Exception as ex:
        main_logger.exception(ex)
...
def main():
    typs_to_create_models_for = ['ResNET50_digit_25PDOUT', 'ResNET50_gender_25PDOUT', 'ResNET50_speaker_25PDOUT']
    class_counts = [10, 1, 20]
    main_logger = setup_logger(f'main_log.log', '/blue/srampazzi/pnaghavi/test30/main_train_1D_CNN_signal_log.log')
    typs_process_list = []
    if torch.cuda.device_count() != 3:
        ...
        for index, type_str in enumerate(typs_to_create_models_for):
            # typs_process_list.append(Process(target=spun_train_one_1D_CNN_signal_classifier, args=('cuda:' + str(index), main_logger, type_str, class_counts[index], index)))
            typs_process_list.append(Process(target=spun_test_one_1D_CNN_signal_classifier, args=('cuda:' + str(index), main_logger, type_str, class_counts[index], index)))
