...
class SpectralDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.data_type != 'train':
            ...
            file_data_1, file_label_1 = self.load_file_data(index)
            file_data_2, file_label_2 = self.load_file_data(random.randint(0, self.__len__() - 1))
            indices = random.sample(range(0, len(file_data_1) * 2), len(file_data_1))
            combined_file_data = file_data_1 + file_data_2
            combined_file_label = file_label_1 + file_label_2
            return [combined_file_data[ii] for ii in indices], [combined_file_label[jj] for jj in indices]
        # return self.load_file_data(index)
...
def spun_train_one_2D_CNN_spectral_classifier(device_instant, main_logger, type_label, num_classes, label_index):
    ...
    DATA_PATH = '/blue/srampazzi/pnaghavi/test38_cross_device/EXT_DATA_AUDIO'
...
def main():
    typs_to_create_models_for = ['HuBERT_pretrained_filtered_DO05_ip12_px5_digit', 'HuBERT_pretrained_filtered_DO05_ip12_px5_gender', 'HuBERT_pretrained_filtered_DO05_ip12_px5_speaker']
