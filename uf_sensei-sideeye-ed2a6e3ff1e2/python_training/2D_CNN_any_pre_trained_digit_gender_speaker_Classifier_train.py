...
def train_a_2D_CNN_spectral_classifier(BASE_PATH, DATA_PATH, logging_instant, device_instant, num_classes, label_index):    
    ...
    model = EfficientNetPretrainedModel(torchvision.models.efficientnet_b7, num_input_channels=8, num_classes=num_classes , dropout=0.001)
    model.to(device)
...
def test_a_2D_CNN_spectral_classifier(BASE_PATH, DATA_PATH, logging_instant, device_instant, num_classes, label_index):    
    ...
    kwargs = {'num_workers': num_workers,
              'pin_memory': True} if use_cuda else {}
    model = EfficientNetPretrainedModel(torchvision.models.efficientnet_b7, num_input_channels=8, num_classes=num_classes , dropout=0.00001)
    model.to(device)
    logging_instant.info(f"{model}")
    logging_instant.info(f"Number of parameters: {count_parameters(model)}")
    test_set = SpectralDataset(DATA_PATH, VALID_BATCH_SIZE, data_type='test', label_index=label_index)
...
def spun_train_one_2D_CNN_spectral_classifier(device_instant, main_logger, type_label, num_classes, label_index):
    BASE_PATH = f'/blue/srampazzi/pnaghavi/test31_order_corrected_makeup/2D_CNN_classifier_{type_label}'
    if not os.path.exists(BASE_PATH):
        os.makedirs(BASE_PATH)
        main_logger.info(f'Created BASE_PATH {BASE_PATH} for label instant {type_label}')
    DATA_PATH = '/blue/srampazzi/pnaghavi/test31_order_corrected_makeup/EXT_DATA_AUDIO_2_ITER'
...
def main():
    typs_to_create_models_for = ['EfficientNet_B7_dropout_2_ITER_pretrained_digit', 'EfficientNet_B7_dropout_2_ITER_pretrained_gender', 'EfficientNet_B7_dropout_2_ITER_pretrained_speaker']
    class_counts = [10, 1, 20]
    main_logger = setup_logger(f'main_log.log', '/blue/srampazzi/pnaghavi/test31_order_corrected_makeup/main_train_2D_CNN_spectral_log_effNet_2_ITER.log')
    typs_process_list = []
